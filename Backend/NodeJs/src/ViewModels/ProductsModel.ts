...
export class ProductsModel {
  ...
  */
  ...
  public static getFilterItems(options: {uid?: string, type?: string, keywords?: string, category?: Array<any>}): Promise<IGetResponse> {
    console.log(options.category, options.uid, options.type);
    if(options.type == 'nocat'){
      return new Promise((resolve, reject) => {
        let res: any[] = [];
        let regex = options.keywords?.split(" ").map(function (e) { return new RegExp(e, "mi"); });
        product.find({"title": { "$in": regex }})
        // .then((val: any) => {res = val; return product.find({"description": { "$regex": options.keywords, "$options": "mi" }});})
        .then((val: any) => {res = val; return product.find({"description": { "$in": regex }});})
        .then((val: any) => {for(let x of val) if(!this.ifin(res, x)) res.push(x); resolve({ "statusCode": 0, "message": "Items retrieved", "payload": res})})
        .catch(e => {reject({ "statusCode": 2, "message": e, "payload": "" })})
      })
    }
    let mquery: { [key: string]: any } = { "categories.name": { "$in": options.category } }
    if (options.uid) {
      mquery.uid = options.uid;
    }
    // {"$in": [{name: "orange"}, {name: "kiwi"}, {name: "mangoes"}, {name: "exoticfruits"}]}
    return new Promise((resolve, reject) => {
