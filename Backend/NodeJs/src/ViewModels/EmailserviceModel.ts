        ...
        UsersModel.userdisabled(body.email).then(userdis => {
          if (body.type === "emailver") {
            if (useren) {
              ...
              if (userdis) {
                EmailserviceModel.sendemail(body.email, "Please verify your email.", "Hi user! \n Welcome to Krishi Bazaar \n Please click on the link below to veryfiy your email.", "https://temp-name-1.herokuapp.com//verifyemail", keys.serverkeys.Secretemail).then(info => {
          ...
          if (body.type === "forgotpass") {
            if (userdis) {
              ...
              if (useren) {
                EmailserviceModel.sendemail(body.email, "Reset Password", "Hi user! \n Welcome to Krishi Bazaar \n Please click on the link below to reset your password.", "https://temp-name-1.herokuapp.com//resetPassword", keys.serverkeys.Secretpassword).then(info => {
