...
export class ConsumerModel {
    ...
    public static addCartCopy(body: I4_0): Promise<IPostResponse>{
        return new Promise((resolve, reject) => {
            let cart = new carts(body);
            let val1: any;
            carts.findOne({ uid: body.uid })
            .then(val => {val1=val; if (val == null) {return cart.save()} else return carts.updateOne({ _id: val._id, "itemsList.productId": body.itemsList.productId }, { "$set": { "itemsList.$.productId": body.itemsList.productId, "itemsList.$.quantity": body.itemsList.quantity } })})
            .then(data => {if (val1 == null){resolve({ statusCode: 0, message: "Product added to cart" })}
                            else {if (data.nModified == 0 && data.n == 0) return carts.updateOne({ _id: val1._id }, { $push: { itemsList: body.itemsList } }); else return Promise.resolve();}})
            .then(val => {resolve({ statusCode: 0, message: "Cart Updated" })})
    ...
    public static deleteCartItem(body: I4_1): Promise<IGetResponse> {
        return new Promise((resolve, reject) => {
            carts.updateOne({ uid: body.uid }, { $pull: { "itemsList": { "productId": body.productId } } }).then(val => {
                return resolve({ "statusCode": 0, "message": "User Cart item deleted", "payload": val });
            })
                .catch(e => {
                    return reject({ "statusCode": 2, "message": e, "payload": "" });
                })
